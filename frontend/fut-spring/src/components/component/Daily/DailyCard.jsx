...
export default function DailyCard({ pelada, onDailySelect, user }) {
  ...
  const handleConfirmPresence = async () => {
    ...
    if (!dailyScheduled || !user || !user.id) {
      console.warn("Cannot confirm presence: missing data.");
      return;
    }
  ...
  const handleDisconfirmPresence = async () => {
    if (submittingAction) return; //
    if (!dailyScheduled || !user || !user.id) {
      console.warn("Cannot disconfirm presence: missing data.");
      return;
    }

    setSubmittingAction('disconfirm');
    try {
      const dailyId = dailyScheduled.id;
      const userId = user.id;
      const url = `/daily/${dailyId}/disconfirm-presence/${userId}`;
      const response = await axiosInstance.put(url, {});

      if (response.status === 200) {
        toast.success(`Presence of "${user.username}" disconfirmed successfully!`);
      } else {
        console.error("API error:", response.data);
      }
    } catch (error) {
      console.error("Error on disconfirming presence:", error);
      toast.error("Failed to disconfirm your presence. Please try again.");
    } finally {
      setSubmittingAction(null); 
    }
  };
  ...
  const handleClick = () => {
    if (dailyScheduled) {
      ...
      console.warn("No scheduled daily found.");
    }
  };
  ...
  const formatDailyTime = (timeString, durationHours) => {
    ...
    tempDate.setHours(startHour, startMinute, 0);
    tempDate.setHours(tempDate.getHours() + durationHours);
    const endTime = `${String(tempDate.getHours()).padStart(2, "0")}:${String(tempDate.getMinutes()).padStart(2, "0")}`;
    return `${startTime}-${endTime}hr`;
  };

  const imageUrl = pelada.image ? `${API_BASE_URL}/pelada/images/${pelada.image}` : null;
    ...
    <div className="flex h-full w-full">
      <Card className="w-full h-full border overflow-hidden p-0 flex flex-col">
        ...
        <CardContent className="flex-1 pt-4 pb-2 px-4">
          <div className="flex justify-between items-center">
            <p className="text-sm font-semibold">
              {pelada.reference}, {dailyScheduled ? formatDailyDate(dailyScheduled.dailyDate) : "Loading data..."}
            </p>
        ...
        <CardFooter className="flex justify-center gap-4 pt-2 pb-4 mt-auto">
          <Button
            className="w-32 font-semibold bg-black text-white hover:!bg-green-600 hover:!border-white"
          ...
          <Button
            className="w-32 font-semibold bg-black text-white hover:!bg-red-900 hover:!border-white"
            onClick={handleDisconfirmPresence}
            disabled={submittingAction !== null}
          >
            {submittingAction === 'disconfirm' ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
              <>
                <X className="h-4 w-4 " />
